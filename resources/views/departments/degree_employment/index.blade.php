                    ...
                    <div class="row">
                        <div class="col text-right">
                            <form action="degree-relevant-employment/0/approve" method="POST">
                                ...
                                <button type="submit"
                                        class="btn btn-sm btn-primary shadow-sm" {{count($employments) == 0 ? 'disabled' : ''}}>
                        ...
                        @if (count($employments) > 0)
                            @foreach ($employments as $employment)
                                <tr role="row" class="odd"
                                    ...
                                    <td class="text-center">
                                        @if(Auth::user()->hasRole('College Super Admin'))
                                            @if($employment->approval_status == "Pending")
                                                <form action="degree-relevant-employment/{{$employment->id}}/approve"
                                                      method="POST">
                                                    @csrf
                                            ...
                                            @endif
                                        @else
                                            @if(!in_array($employment->approval_status, ["Approved", "College Approved"]))
                                                <div class="row px-1">
                                                    <div class="col px-0">
                                                        <form class="p-0"
                                                    ...
                                                    <div class="col px-0">
                                                        <button type="submit"
                                                                ...
                                                                style="opacity:0.80" data-id="{{$employment->id}}"
                                    ...
                                    <td>{{$employment->female_students_number + $employment->male_students_number}}</td>
                                    ...
                                    @else
                                        <td class="text-danger"><i
                                                    class="fas fa-times"></i> {{$employment->approval_status}}</td>
                                    @endif
                                </tr>
                            @endforeach
                        @endif
